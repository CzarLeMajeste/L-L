...
import { Home, Building2, Search, SlidersHorizontal } from 'lucide-react'
    ...
    <div>
      <section className="relative mb-8 overflow-hidden rounded-3xl bg-gradient-to-br from-brand-700 via-brand-600 to-brand-800 px-6 py-12 text-white shadow-lift sm:px-10 sm:py-16">
        ...
        <div className="relative">
          <span className="chip bg-white/15 text-white backdrop-blur-sm">Love at the Lodge</span>
          <h1 className="mt-4 max-w-2xl font-display text-3xl font-extrabold leading-tight tracking-tight sm:text-5xl">
            Find your next stay among Philippine lodging houses & private condos
          </h1>
          <p className="mt-3 max-w-xl text-sm text-brand-50/90 sm:text-base">
            From cool mountain lodges in Baguio to bright city condos in Makati — book in minutes and pay with InstaPay QR.
          </p>
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div className="flex gap-2">
            <FilterPill active={!filter} onClick={() => onFilter(undefined)} label="All stays" />
            <FilterPill
            ...
            />
            <FilterPill
              active={filter === 'PRIVATE_CONDO'}
              onClick={() => onFilter('PRIVATE_CONDO')}
              label="Private Condos"
      ...
      {listings && filtered.length === 0 && !error && (
        <div className="card animate-fade-up p-10 text-center">
          <p className="text-sm font-semibold text-ink-600">No stays match your search.</p>
          <p className="mt-1 text-xs text-ink-400">Try a different city or clear the filters.</p>
        </div>
      )}
