...
interface Reply { id: string; author: string; body: string; createdAt: string }
interface Post { id: string; author: string; title: string; body: string; votes: number; comments: number; replies?: Reply[]; createdAt: string; reported?: boolean }
...
export function CommunityView({ listing, profile, onBack, onSignIn, onOpenCommunity }: Props) {
  ...
  const createPost = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const nextPost = { id: crypto.randomUUID(), author: channel === 'rants' ? 'Anonymous boarder' : profile.displayName, title: title.trim(), body: body.trim(), votes: 1, comments: 0, replies: [], createdAt: 'Just now' }
    save([nextPost, ...activePosts]); setTitle(''); setBody(''); localStorage.setItem('boarder-last-post', String(Date.now())); setCooldown(Date.now())
  }

  const createReply = (postId: string) => {
    setError(null)
    if (!profile?.verified) return setError('Verify your student ID before replying.')
    if (channel === 'private' && !privateChannelAccess) return setError('Only verified boarders can reply in this channel.')
    if (replyDraft.trim().length < 3) return setError('Write a little more before replying.')
    if (Date.now() - cooldown < 30_000) return setError('Please wait a little before replying again.')
    const nextReply: Reply = { id: crypto.randomUUID(), author: channel === 'rants' ? 'Anonymous boarder' : profile.displayName, body: replyDraft.trim(), createdAt: 'Just now' }
    save(activePosts.map((post) => post.id === postId ? { ...post, comments: post.comments + 1, replies: [...(post.replies ?? []), nextReply] } : post))
    setReplyDraft('')
    localStorage.setItem('boarder-last-post', String(Date.now()))
  ...
  return (
    <div className="animate-fade-up">
      ...
      {(channel === 'public' || channel === 'rants' || privateChannelAccess) && <div className="space-y-3">{sortedPosts.map((post) => <article key={post.id} className="card overflow-hidden transition-shadow hover:shadow-soft">
        <div className="p-5"><div className="flex gap-4"><div className="flex shrink-0 flex-col items-center gap-1 text-ink-400"><button onClick={() => save(activePosts.map((item) => item.id === post.id ? { ...item, votes: item.votes + 1 } : item))} aria-label="Upvote post"><ThumbsUp className="h-4 w-4" /></button><span className="text-xs font-bold text-ink-700">{post.votes}</span></div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><button onClick={() => { setExpandedThread(expandedThread === post.id ? null : post.id); setError(null) }} className="text-left"><h2 className="font-display text-base font-bold text-ink-900 hover:text-brand-700">{post.title}</h2><p className="mt-1 text-[11px] text-ink-400">{post.author} · {post.createdAt}</p></button><button onClick={() => save(activePosts.map((item) => item.id === post.id ? { ...item, reported: true } : item))} className="text-ink-300 hover:text-red-500" aria-label="Report post"><Flag className="h-4 w-4" /></button></div><p className="mt-3 text-sm leading-relaxed text-ink-600">{post.body}</p><button onClick={() => { setExpandedThread(expandedThread === post.id ? null : post.id); setError(null) }} className="mt-4 flex items-center gap-1 text-xs font-semibold text-ink-400 hover:text-brand-700"><MessageCircle className="h-3.5 w-3.5" /> {post.comments} comments · {expandedThread === post.id ? 'Hide thread' : 'Open thread'}</button>{post.reported && <p className="mt-2 text-[11px] text-red-600">Reported for moderator review.</p>}</div></div></div>
        {expandedThread === post.id && <div className="border-t border-ink-100 bg-ink-50/60 p-5"><div className="space-y-3">{(post.replies ?? []).map((reply) => <div key={reply.id} className="rounded-xl bg-white p-3"><p className="text-[11px] font-semibold text-ink-400">{reply.author} · {reply.createdAt}</p><p className="mt-1 text-sm text-ink-600">{reply.body}</p></div>)}{(post.replies ?? []).length === 0 && <p className="text-xs text-ink-400">No replies yet. Start the thread.</p>}</div>{profile && (channel === 'public' || channel === 'rants' || privateChannelAccess) && <div className="mt-4 flex gap-2"><input className="input" value={replyDraft} onChange={(event) => setReplyDraft(event.target.value)} placeholder="Reply to this thread..." maxLength={500} /><button onClick={() => createReply(post.id)} className="btn-primary shrink-0 px-3" aria-label="Reply to thread"><Send className="h-4 w-4" /></button></div>}{error && expandedThread === post.id && <p className="mt-2 text-xs font-medium text-red-600">{error}</p>}</div>}
      </article>)}</div>}
        </div>
